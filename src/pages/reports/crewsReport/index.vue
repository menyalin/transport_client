      ...
      <table ref="tableBody" class="background-table">
        ...
        <tbody class="table-body">
          <tr v-for="row in tableRows" :key="row._id">
            ...
            <td v-for="day in tableColumns" :key="day.title" class="data-cell"></td>
          </tr>
...
export default {
  ...
  methods: {
    getUrlForRowTitle(id) {
      if (this.settings.group === 'truck' || this.settings.group === 'trailer') return '/profile/trucks/' + id
    ...
    resizeHandler() {
      ...
      this.$nextTick(() => {
        ...
        const dSec = dayjs(this.settings.period[1]).add(24, 'hour').unix() - dayjs(this.settings.period[0]).unix()
    ...
    getWidthInPxForBlock(block) {
      ...
      if (!block.endDate || dayjs(this.settings.period[1]).add('24', 'hour').isSameOrBefore(block.endDate))
