<template>
  <div>
    <ButtonsPanel
      panelType="form"
      :disabledSubmit="!$store.getters.hasPermission('fine:write') || isInvalidForm"
      class="mb-4"
      @cancel="cancel"
      @submit="submit"
    />
    ...
    <v-text-field v-model.trim="$v.form.violation.$model" label="Нарушение" variant="outlined" />
    density="compact"
    ...
    <div class="row-input">
      ...
      <v-autocomplete
        ...
        density="compact"
        :style="{ maxWidth: '350px' }"
        :appendIcon="form.truck && form.violationDate ? 'mdi-crosshairs' : null"
        @click:append="getDriver"
      />
    </div>

    <v-text-field v-model="$v.form.address.$model" label="Место нарушения" variant="outlined" />
    density="compact"
    <div class="row-input">
    ...
    <v-text-field v-model="$v.form.note.$model" label="Примечание" variant="outlined" />
    density="compact"
