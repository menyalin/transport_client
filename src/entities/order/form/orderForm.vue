    ...
    <v-row>
      <v-col>
        <div class="top-panel">
          ...
          <div class="template-panel">
            <v-autocomplete
              ...
              :style="{ width: '350px' }"
            />
            <v-btn color="primary" :disabled="isInvalidForm || !!templateSelector" @click="templateDialog = true">
              Создать шаблон
            </v-btn>
            ...
            <v-dialog v-model="templateDialog" persistent maxWidth="600">
              <v-card>
        ...
        <div class="wrapper">
          ...
          <div id="price">
            <AppAnalyticBlock v-model="analytics" :isValidRoute="isValidRoute" :coords="coords" title="Аналитика" />

            <AppPaymentToDriver v-if="showPaymentToDriver" id="payment-to-driver" v-model="paymentToDriver" />
          ...
          <div id="note">
            <v-text-field v-model="form.note" variant="outlined" label="Примечание" density="compact" />
            <v-text-field
          ...
          <EntityFiles v-if="order && order._id" id="order-files" :itemId="order._id" docType="order" />
          <OrderDocsListForm
            v-if="isShowDocs"
            id="docs"
            v-model="docs"
...
<script>
  ...
  export default {
    ...
    setup(props) {
      const { templates, docTemplateIsVisible, downloadTemplateHandler, downloadDisabled } = useOrderPrintForms({
        order: props.order,
      })
      ...
      const hasPaymentInvoices = computed(() => {
        return props.order?.paymentInvoices && props.order?.paymentInvoices.length > 0
      })
    ...
    computed: {
      showPaymentToDriver() {
        return this.$store.getters.hasPermission('order:readPaymentToDriver') && !this.confirmedCrew.outsourceAgreement
      },
    ...
    watch: {
      'confirmedCrew.outsourceAgreement': {
        handler: async function (val) {
          this.carrierAgreement = val ? await CarrierAgreementService.getById(val) : null
        },
        immediate: true,
      },
      ...
      route: {
        ...
        handler: function (newRouteValue, oldVal) {
          ...
          if (this.isValidRoute && newRouteValue.map(r => r.address).join() !== oldVal.map(r => r.address).join()) {
    ...
    methods: {
      ...
      async submit(_val, saveOnly) {
        ...
        if (!this.analytics.distanceDirect) this.analytics.distanceDirect = OrderService.getDirectDistance(this.coords)
        ...
        this.prePrices = this.updatePricesUseAgreement(this.prePrices, this.agreement)
        this.prices = this.updatePricesUseAgreement(this.prices, this.agreement)
        this.finalPrices = this.updatePricesUseAgreement(this.finalPrices, this.agreement)
