<template>
  <v-dialog :modelValue="dialog" maxWidth="800" persistent @update:model-value="closeDialog">
    <v-card>
      ...
      <form @submit.prevent="submitHandler">
        <v-card-text class="form-wrapper">
          ...
          <div class="row-sum">
            ...
            />
            <v-checkbox v-model="state.sumWithVAT" label="Сумма с НДС" dense :disabled="vatCheckboxIsDisabled" />
          </div>
          <v-text-field v-model.trim="state.note" label="Примечание" variant="outlined" density="compact" />
        </v-card-text>

        <v-card-actions>
          <v-spacer />
          <v-btn color="primary" variant="text" @click="closeDialog">Отмена</v-btn>
          <v-btn color="primary" variant="text" type="submit" :disabled="invalidForm">Добавить</v-btn>
        </v-card-actions>
      </form>
    </v-card>
  </v-dialog>
...
<script>
  ...
  export default {
    ...
    setup(props, ctx) {
      ...
      const { state, clientItems, agreements, vatCheckboxIsDisabled, sumFieldIsDisabled, submitHandler, invalidForm } =
