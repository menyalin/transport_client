<template>
  <v-tooltip location="bottom" nudgeLeft="70" openDelay="700"
close-delay="50" >
    ...
    <div>
      <div v-for="point in points.filter(p => p.type === 'loading')" :key="point.idx" class="title-row-text">
        {{ point.title }}
      </div>
      <div v-for="point in points.filter(p => p.type === 'unloading')" :key="point.idx" class="title-row-text">
        {{ point.title }}
      </div>
    </div>
  </v-tooltip>
...
export default {
  ...
  computed: {
    breakingSchedule() {
      ...
      roundedStartPositionDate = roundedStartPositionDate.hour(roundingHours(roundedStartPositionDate.hour()))
    ...
    orderClasses() {
      ...
      if (this.order.state.status === 'notСonfirmedByClient') classes.push('not-confirmed-by-client')
    ...
    delayToPointInd() {
      if (['notСonfirmedByClient', 'weRefused', 'clientRefused', 'needGet'].includes(this.order.state.status))
        return -1
