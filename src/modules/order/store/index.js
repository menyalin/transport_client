...
const _getLastPositionDate = order => {
  const tmpRoute = order.route.slice()
  if (tmpRoute[tmpRoute.length - 1].departureDate) return tmpRoute[tmpRoute.length - 1].departureDate
  let routeDates = []
...
export default {
  ...
  getters: {
    ...
    orderStatusesMap: ({ orderStatuses }) => new Map(orderStatuses.map(item => [item.value, item.text])),
    orderAnalyticTypes: ({ orderAnalyticTypes }) => orderAnalyticTypes,
    orderAnalyticTypesMap: ({ orderAnalyticTypes }) => new Map(orderAnalyticTypes.map(item => [item.value, item.text])),
    ...
    orderCountByDates: ({ orders }) => {
      ...
      filteredOrders.forEach(order => {
        ...
        const newDay = {
          ...
          [zone]: tmpRes.has(dayStr) && !!tmpRes.get(dayStr)[zone] ? tmpRes.get(dayStr)[zone] + 1 : 1,
        }
    ...
    orderPriceTypesMap: ({ orderPriceTypes }) => new Map(orderPriceTypes.map(t => [t.value, t.text])),
  },
}
